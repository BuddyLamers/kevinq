...
function foo_video_import_video_as_attachment( $video_object ) {
	...
	$response = wp_remote_get( $video_object[ 'thumbnail'] );
	...
	$thumbnail_filename = $video_object['id'] . '.' . pathinfo( basename( $video_object[ 'thumbnail' ] ), PATHINFO_EXTENSION );
	...
	$thumbnail = wp_get_attachment_image_src( $attachment_id, 'thumbnail' ); // Yes we have the data, but get the thumbnail URL anyway, to be safe
