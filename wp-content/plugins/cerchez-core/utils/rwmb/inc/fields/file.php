	...
	class RWMB_File_Field extends RWMB_Field {
		...
		static function admin_enqueue_scripts() {
			...
			wp_localize_script('rwmb-file', 'rwmbFile', array(
				'maxFileUploadsSingle' => __('You may only upload maximum %d file', 'cerchez-core'),
				'maxFileUploadsPlural' => __('You may only upload maximum %d files', 'cerchez-core'),
			) );
		}
		...
		static function wp_ajax_delete_file() {
			...
			if ( $ok )
				...
				wp_send_json_error( __('Error: Cannot delete file', 'cerchez-core') );
		}

		static function html( $meta, $field ) {
			$i18n_title = apply_filters('rwmb_file_upload_string', _x('Upload Files', 'file upload', 'cerchez-core'), $field );
			$i18n_more  = apply_filters('rwmb_file_add_string', _x('+ Add new file', 'file upload', 'cerchez-core'), $field );
		...
		static function file_html( $attachment_id ) {
			$i18n_delete = apply_filters('rwmb_file_delete_string', _x('Delete', 'file upload', 'cerchez-core') );
			$i18n_edit   = apply_filters('rwmb_file_edit_string', _x('Edit', 'file upload', 'cerchez-core') );
			$li = '
			<li>
		...
		static function value( $new, $old, $post_id, $field ) {
			$name = $field['id'];
			if ( empty( $_FILES[ $name ] ) ) return $new;
