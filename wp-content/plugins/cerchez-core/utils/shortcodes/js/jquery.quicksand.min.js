...
var cerchez_filter_container_data = {};



function triggerCerchezQuickSandInit(target) {

	jQuery('.cerchez-filter-selectors:not(.cerchez-filter-init)', target).each(function() {

		$filter_selector = jQuery(this);

		if ($filter_selector.hasClass('cerchez-filter-init')) return;



		$filter_selector.addClass('cerchez-filter-init');
		...
		cerchez_filter_container_data[filter_container.attr('id')] = filter_container.clone();
		...
		jQuery('a', $filter_selector).on("click", function(e) {
			...
			var filter_container = $this.parents('.cerchez-filter-selectors').data('container');

			var is_showcase = (cerchez_filter_container_data[ filter_container ].find('.portfolio-showcase').length > 0);

			var filter_selector;

			if (filter == 'all') {

				filter_selector = '.col, .portfolio-showcase .thumb';

			} else {

				filter_selector = '.col[data-type~=' + filter + '], .portfolio-showcase .thumb[data-type~=' + filter + ']';

			}

			var $filteredData = cerchez_filter_container_data[ filter_container ].find(filter_selector);

			var final_quicksand_selector = '#' + filter_container;

			if ( ! is_showcase ) {

				var filter_container_cols = $this.parents('.cerchez-filter-selectors').data('columns');

				$filteredData.removeClass('alpha omega').each(function(index) {

					if (index % filter_container_cols == 0) jQuery(this).addClass('alpha');

					if (index % filter_container_cols == (filter_container_cols-1)) jQuery(this).addClass('omega');

				});

			} else {

				final_quicksand_selector = final_quicksand_selector + ' .portfolio-showcase';
...
jQuery(document).on("cerchezWidgetUpdate", function(e){
	...
	triggerCerchezQuickSandInit(e.target);
